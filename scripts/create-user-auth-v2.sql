-- Create user authentication system for approved waitlist members (with conflict handling)

-- Create approved_users table (with IF NOT EXISTS)
...
-- Create user sessions table (with IF NOT EXISTS)
...
-- Create trigger to auto-create approved users (drop and recreate to avoid conflicts)
...
-- Add updated_at trigger for approved_users (drop and recreate to avoid conflicts)
