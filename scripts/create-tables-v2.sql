-- Create tables for THC Club admin system (with IF NOT EXISTS checks)
...
CREATE INDEX IF NOT EXISTS idx_shelf_slots_status ON shelf_slots(status);
CREATE INDEX IF NOT EXISTS idx_shelf_slots_type ON shelf_slots(shelf_type);

-- Create updated_at triggers function (only if it doesn't exist)
...
DROP TRIGGER IF EXISTS update_shelf_slots_updated_at ON shelf_slots;
