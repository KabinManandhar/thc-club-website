...
import { AlertCircle, CircleDollarSign, FileText, Package, RefreshCw } from "lucide-react"
                  ...
                  <div className="bg-gray-50 rounded-[2.5rem] border border-gray-100 p-8 flex flex-col">
                     ...
                     {currentInvoices.length === 0 ? (
                        ...
                        <div className="space-y-4 overflow-y-auto max-h-[200px] pr-2">
                           {currentInvoices.slice(0, 10).map((inv: any) => (
                              <div key={inv.id} className="flex flex-col gap-2 pb-4 border-b border-gray-200 last:border-0 last:pb-0">
                                 ...
                                 {inv.invoice_line_items && inv.invoice_line_items.length > 0 && (
                                    <div className="bg-white/50 p-3 rounded-xl border border-gray-100 flex flex-col gap-1.5">
                                       {inv.invoice_line_items.map((item: any) => (
                                          <div key={item.id} className="flex justify-between items-start text-[10px] leading-tight">
                                             <span className="font-bold text-gray-600 max-w-[70%] line-clamp-2">{item.quantity}x {item.product_name}</span>
                                             <span className="font-black text-gray-400">NPR {item.line_total.toLocaleString()}</span>
                                          </div>
                                       ))}
                                       <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest text-[#FE7F2D] border-t border-gray-100 pt-1.5 mt-0.5">
                                          <span>platform fee</span>
                                          <span>- NPR {inv.ppf_amount?.toLocaleString() || '0'}</span>
                                       </div>
                                    </div>
                                 )}
                              </div>
